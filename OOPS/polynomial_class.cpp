...
class Polynomial {
    ...
    Polynomial operator+(Polynomial const &p2){
        ...
        while(i<this->size && j<p2.size){
            ...
            fp.setCoefficient(degree, coeff);
            i++;
            j++;
        }
        while(i<size){
            fp.setCoefficient(i, degCoeff[i]);
            i++;
        }
        while(j<p2.size){
            fp.setCoefficient(j, p2.degCoeff[j]);
            j++;
        }
        return fp;
    }
    ...
    Polynomial operator-(Polynomial p2){
        ...
        while(j<p2.size){
            fp.setCoefficient(j, (-1 * p2.degCoeff[j]));
            j++;
        }
        return fp;
    }
    ...
    Polynomial operator*(Polynomial p2){
        ...
        for(int j=0; j<p2.size; j++){
            for(int i=0; i<size; i++){
                int degree=i+j;
                int coeff=fp.getCoeff(degree) + (this->degCoeff[i] * p2.degCoeff[j]);
                fp.setCoefficient(degree, coeff);
            }
        }
        return fp;
